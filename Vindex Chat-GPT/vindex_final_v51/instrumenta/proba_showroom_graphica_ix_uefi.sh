#!/usr/bin/env bash
# Graphica IX: showroom rasterae premium sub QEMU/OVMF exercet.
set -euo pipefail

RADIX="$(cd "$(dirname "$0")/.." && pwd)"
UEFI="$RADIX/systema/uefi"
TEMPORARIUM="$(mktemp -d "${TMPDIR:-/tmp}/vindex-graphica-ix.XXXXXX")"
...
if ! command -v qemu-system-x86_64 >/dev/null 2>&1 || [ -z "$OVMF_CODE" ] || [ -z "$OVMF_VARS" ]; then
    printf '%s\n' 'OMISSUM: QEMU/OVMF deest; showroom Graphica IX saltatur.'
    exit 0
fi

printf '%s\n' 'I. Showroom Graphica IX ut NUCLEUS.BIN construe...'
NUCLEUS_FONS="$RADIX/systema/showroom_graphica_ix_uefi.vindex" \
...
printf '%s\n' 'II. Showroom Graphica IX sub QEMU/OVMF exerce...'
...
if ! python3 "$RADIX/instrumenta/proba_showroom_graphica_ix.py" "$MONITOR" "$TEMPORARIUM" "$MORA_INITII"; then
...
printf '%s\n' '=== GRAPHICA IX SHOWROOM UEFI PROBATUM ==='
printf '%s\n' 'OVMF -> BOOTX64.EFI [VINDEX] -> SHOWROOM [VINDEX] -> GRAPHICA IX -> FRAMEBUFFER'
