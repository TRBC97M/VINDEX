#!/usr/bin/env bash
# P16-XII-C2: SIMG II -> Graphica IX -> superficies GX 9-slice sub QEMU/OVMF.
set -euo pipefail

RADIX="$(cd "$(dirname "$0")/.." && pwd)"
UEFI="$RADIX/systema/uefi"
TEMPORARIUM="$(mktemp -d "${TMPDIR:-/tmp}/vindex-asseta-gx-novem-x.XXXXXX")"
...
if ! command -v qemu-system-x86_64 >/dev/null 2>&1 || [ -z "$OVMF_CODE" ] || [ -z "$OVMF_VARS" ]; then
    printf '%s\n' 'OMISSUM: QEMU/OVMF deest; showroom Assetorum GX Novem saltatur.'
    exit 0
fi

printf '%s\n' 'I. Showroom Assetorum GX Novem ut NUCLEUS.BIN construe...'
NUCLEUS_FONS="$RADIX/systema/showroom_asseta_gx_novem_x_uefi.vindex" \
...
grep -qa 'NUCLEUS BIN' "$TEMPORARIUM/systema.img" || {
    printf '%s\n' 'DEFECIT: showroom Assetorum GX Novem in imagine deest.' >&2
    exit 2
}
...
printf '%s\n' 'II. Asseta GX Novem sub QEMU/OVMF exerce...'
...
if ! python3 "$RADIX/instrumenta/proba_showroom_asseta_gx_novem_x.py" "$MONITOR" "$TEMPORARIUM" "$MORA_INITII"; then
...
printf '%s\n' '=== ASSETA GX NOVEM UEFI PROBATA ==='
printf '%s\n' 'OVMF -> VINDEX -> SIMG II -> Graphica IX -> GX 9-slice -> scena -> framebuffer'
