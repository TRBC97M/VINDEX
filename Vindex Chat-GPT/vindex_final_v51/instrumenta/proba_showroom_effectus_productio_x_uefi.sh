#!/usr/bin/env bash
# P16-XII-C1: effectus productionis Graphica X sub QEMU/OVMF probantur.
set -euo pipefail

RADIX="$(cd "$(dirname "$0")/.." && pwd)"
UEFI="$RADIX/systema/uefi"
TEMPORARIUM="$(mktemp -d "${TMPDIR:-/tmp}/vindex-effectus-productio-x.XXXXXX")"
...
if ! command -v qemu-system-x86_64 >/dev/null 2>&1 || [ -z "$OVMF_CODE" ] || [ -z "$OVMF_VARS" ]; then
    printf '%s\n' 'OMISSUM: QEMU/OVMF deest; showroom Effectuum Productionis saltatur.'
    exit 0
fi

printf '%s\n' 'I. Showroom effectuum productionis ut NUCLEUS.BIN construe...'
NUCLEUS_FONS="$RADIX/systema/showroom_effectus_productio_x_uefi.vindex" \
...
grep -qa 'NUCLEUS BIN' "$TEMPORARIUM/systema.img" || {
    printf '%s\n' 'DEFECIT: showroom Effectuum Productionis in imagine deest.' >&2
    exit 2
}
...
printf '%s\n' 'II. Effectus productionis sub QEMU/OVMF exerce...'
...
if ! python3 "$RADIX/instrumenta/proba_showroom_effectus_productio_x.py" "$MONITOR" "$TEMPORARIUM" "$MORA_INITII"; then
...
printf '%s\n' '=== EFFECTUS PRODUCTIONIS X UEFI PROBATI ==='
printf '%s\n' 'OVMF -> VINDEX -> cache -> backdrop/maska/clip/gloss -> scena GX -> framebuffer'
