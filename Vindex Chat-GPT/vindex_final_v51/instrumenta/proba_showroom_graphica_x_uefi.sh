#!/usr/bin/env bash
# P16-XII: Graphica X sub QEMU/OVMF in framebuffer vero probatur.
set -euo pipefail

RADIX="$(cd "$(dirname "$0")/.." && pwd)"
UEFI="$RADIX/systema/uefi"
TEMPORARIUM="$(mktemp -d "${TMPDIR:-/tmp}/vindex-graphica-x.XXXXXX")"
...
if ! command -v qemu-system-x86_64 >/dev/null 2>&1 || [ -z "$OVMF_CODE" ] || [ -z "$OVMF_VARS" ]; then
    printf '%s\n' 'OMISSUM: QEMU/OVMF deest; showroom Graphica X saltatur.'
    exit 0
fi

printf '%s\n' 'I. Showroom Graphica X ut NUCLEUS.BIN construe...'
NUCLEUS_FONS="$RADIX/systema/showroom_graphica_x_uefi.vindex" \
...
grep -qa 'NUCLEUS BIN' "$TEMPORARIUM/systema.img" || {
    printf '%s\n' 'DEFECIT: showroom Graphica X in imagine deest.' >&2
    exit 2
}
...
printf '%s\n' 'II. Graphica X sub QEMU/OVMF exerce...'
...
if ! python3 "$RADIX/instrumenta/proba_showroom_graphica_x.py" "$MONITOR" "$TEMPORARIUM" "$MORA_INITII"; then
...
printf '%s\n' '=== GRAPHICA X UEFI PROBATA ==='
printf '%s\n' 'OVMF -> VINDEX -> GX RGBA -> blur/vitrum/umbra -> framebuffer'
