...
def principale() -> int:
    if len(sys.argv) not in (3, 4, 5, 6):
        print(
            "USUS: fac_imaginem_uefi.py BOOTX64.EFI imago.img [NUCLEUS.BIN] [TEXTUS.BIN] [FORMA.BIN]",
            file=sys.stderr,
        )
        return 64
    ...
    forma = Path(sys.argv[5]).read_bytes() if len(sys.argv) == 6 else b""
    ...
    forma_botri = (len(forma) + SECTOR - 1) // SECTOR if forma else 0
    primus_formae = (ultimus_textus if textus else (
        ultimus_nuclei if nucleus else ultimus_fasciculi)) + 1
    ultimus_formae = primus_formae + forma_botri - 1
    voluminis_botri = MENSURA_VOLUMINIS // SECTOR
    ultimus_ante_volumen = ultimus_formae if forma else (
        ultimus_textus if textus else (
    ...
    si_occupati = 3 + fasciculi_botri + nuclei_botri + textus_botri + forma_botri + voluminis_botri
    ...
    for botrus in range(primus_textus, ultimus_textus + 1):
        ...
        u32(fat, botrus * 4, proximus)
    for botrus in range(primus_formae, ultimus_formae + 1):
        proximus = 0x0FFFFFFF if botrus == ultimus_formae else botrus + 1
        u32(fat, botrus * 4, proximus)
    ...
    if forma:
        radix[128:160] = ingressus_directorii(
            b"FORMA   BIN", 0x20, primus_formae, len(forma)
        )
    ...
    if forma:
        debut_formae = locus_botri(primus_formae)
        imago[debut_formae:debut_formae + len(forma)] = forma
