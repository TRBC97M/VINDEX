#!/usr/bin/env bash
# Sylvia OS — Fenestrale II, Gradus E.
# Systema 0.51 idem nucleum servat, sed firmamentum ABI Fenestralis II exponit.

set -eu

RADIX="$(cd "$(dirname "$0")/../.." && pwd)"
UEFI="$RADIX/systema/uefi"
IMAGO="${1:-$RADIX/systema_vindex_uefi_fenestrale_e.img}"
APPLICATIO="${2:-$RADIX/BOOTX64-FENESTRALE-E.EFI}"
TEMPORARIUM="$(mktemp -d "${TMPDIR:-/tmp}/vindex-fenestrale-e.XXXXXX")"
...
python3 "$UEFI/genera_firmamentum_fenestrale_e.py" \
    "$UEFI/firmamentum_uefi.c" "$TEMPORARIUM/firmamentum_fenestrale_e.c"
...
gcc -c -std=c11 -O2 -Wall -Wextra -Werror -ffreestanding -fno-builtin \
    ...
    -I"$RADIX/systema" \
    "$TEMPORARIUM/firmamentum_fenestrale_e.c" \
    -o "$TEMPORARIUM/firmamentum.o"
...
python3 "$UEFI/fac_imaginem_uefi.py" \
    "$TEMPORARIUM/BOOTX64.EFI" "$TEMPORARIUM/systema_vindex_uefi_fenestrale_e.img"

mkdir -p "$(dirname "$IMAGO")" "$(dirname "$APPLICATIO")"
cp -f "$TEMPORARIUM/BOOTX64.EFI" "$APPLICATIO"
cp -f "$TEMPORARIUM/systema_vindex_uefi_fenestrale_e.img" "$IMAGO"
chmod 0644 "$APPLICATIO" "$IMAGO"
printf '%s\n' 'RECTE: VINDEX 0.51 cum ABI Fenestralis II Gradus E constructum est.'
printf 'APPLICATIO: %s\n' "$APPLICATIO"
printf 'IMAGO: %s\n' "$IMAGO"
