...
# Compositorium multi-client cum mure, PROGRAMMATA H et TABULA runtime construit.

set -eu
RADIX="$(cd "$(dirname "$0")/../.." && pwd)"
UEFI="$RADIX/systema/uefi"
EXITUS="${1:-$RADIX/FENESTRALEJ.EFI}"
IMAGO="${2:-$RADIX/fenestrale_j_uefi.img}"
TEMPORARIUM="$(mktemp -d "${TMPDIR:-/tmp}/sylvia-fenestrale-j.XXXXXX")"
...
gcc -c -std=c11 -O2 -Wall -Wextra -Werror -Wno-error=misleading-indentation \
    ...
    -I "$RADIX/systema" "$UEFI/fenestrale_native_j.c" \
    -o "$TEMPORARIUM/fenestrale_native_j.o"
objcopy --remove-section .comment --remove-section .note.GNU-stack "$TEMPORARIUM/fenestrale_native_j.o"

ld -mi386pep --subsystem 10 --entry efi_main --image-base 0x16000000 \
    --no-insert-timestamp --stack 0x200000 --section-alignment 4096 --file-alignment 512 \
    "$TEMPORARIUM/fenestrale_native_j.o" "$TEMPORARIUM/programmata_h.o" "$TEMPORARIUM/tabula_i.o" \
    -o "$TEMPORARIUM/FENESTRALEJ.EFI"

file "$TEMPORARIUM/FENESTRALEJ.EFI" | grep -q 'PE32+.*EFI'
objdump -p "$TEMPORARIUM/FENESTRALEJ.EFI" | grep -q 'Subsystem.*EFI application'
python3 "$UEFI/fac_imaginem_uefi.py" "$TEMPORARIUM/FENESTRALEJ.EFI" "$TEMPORARIUM/fenestrale_j_uefi.img"
mkdir -p "$(dirname "$EXITUS")" "$(dirname "$IMAGO")"
cp -f "$TEMPORARIUM/FENESTRALEJ.EFI" "$EXITUS"
cp -f "$TEMPORARIUM/fenestrale_j_uefi.img" "$IMAGO"
chmod 0644 "$EXITUS" "$IMAGO"
printf '%s\n' 'RECTE: Fenestrale II Gradus J constructum est.'
printf 'APPLICATIO: %s\nIMAGO: %s\n' "$EXITUS" "$IMAGO"
