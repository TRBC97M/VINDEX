...
class ErreurLexicale(Exception):
    def __init__(self, message, ligne, colonne):
        super().__init__(f"Erratum lexicale L{ligne}:C{colonne} — {message}")
...
class Lexeur:
    ...
    def _lire_chaine(self):
        ...
        if self._caractere_actuel() is None:
            raise ErreurLexicale("catena litterarum non terminata", ligne, colonne)
        valeur = self.source[debut:self.position]
    ...
    def _lire_caractere(self):
        ...
        c = self._caractere_actuel()
        if c is None:
            raise ErreurLexicale("littera non terminata", ligne, colonne)
        if c == "\\":
            self._avancer()
            echappe = self._caractere_actuel()
            if echappe not in self._ECHAPPEMENTS:
                raise ErreurLexicale(f"series effugii ignota '\\{echappe}'", ligne, colonne)
            ...
            self._avancer()
        if self._caractere_actuel() != "'":
            raise ErreurLexicale("apostrophus claudens exspectatur", ligne, colonne)
        self._avancer()
    ...
    def tokeniser(self):
        while True:
            ...
            if c.isdigit():
                ...
                raise ErreurLexicale(f"littera inexspectata '{c}'", ligne, colonne)
