...
class ErreurSyntaxique(Exception):
    def __init__(self, message, token):
        pos = f"L{token.ligne}:C{token.colonne}" if token else "fin de fichier"
        super().__init__(f"Erreur syntaxique {pos} — {message} (reçu: {token})")
...
class Analyseur:
    ...
    def _attendre(self, type_token, message=None):
        if not self._verifier(type_token):
            raise ErreurSyntaxique(message or f"attendu {type_token.name}", self._actuel())
    ...
    def _analyser_type(self):
        ...
        if self._actuel().type not in TOKENS_DE_TYPE:
            raise ErreurSyntaxique("type attendu", self._actuel())
    ...
    def _analyser_instruction(self):
        ...
        raise ErreurSyntaxique("instruction attendue", self._actuel())
    ...
    def _analyser_primaire(self):
        ...
        raise ErreurSyntaxique("expression attendue", t)
