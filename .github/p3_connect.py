...
novum_meta = """    // VI-B. Moderatores firmware recursive coniunge. Hoc gradum laboratorii #71
    // e C in ponticulum VINDEX purum transfert, ut etiam PS/2 Simple Pointer
    // ante ingressum nuclei a firmware publicari possit.
...
textus = textus.replace(
    ancora_imago,
    ancora_imago + "    CONTENTUM(50334504) = moderatores_n.\n",
    1,
)
...
proba = proba.replace(
    ...
    "#   V.   Sylvia in schermo vere pingit (screendump inspectus).\n#   VI.  murus PS/2 per firmware coniunctum Sylviam vere movet.\n",
    1,
)
...
nuntia 'VI. Murus PS/2 per VINDEX purum...'
...
if ! python3 "$RADIX/instrumenta/proba_murem_uefi_053.py" \\
    ...
    defecit 'murus PS/2 Sylviam non movit' 6
fi
wait "$PID_MUS" 2>/dev/null || true
nuntia '   RECTE: murus PS/2 in catena canonica movetur.'
nuntia ''
nuntia '=== CATENA UEFI INTEGRA PROBATA ==='
nuntia 'OVMF -> BOOTX64.EFI [VINDEX] -> NUCLEUS [VINDEX] -> FRAMEBUFFER -> INPUT PS/2 -> SYLVIA'
nuntia 'Nullum C in tota via.'
"""
...
print('RECTE: ConnectController, metadata et probatio PS/2 inserta sunt.')
